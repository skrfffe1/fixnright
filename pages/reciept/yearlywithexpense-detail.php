...
<head>
  ...
  <link href="/dist/output.css" rel="stylesheet">
	...
	<center><header style="font-size:25px;background-color: greenyellow;padding: 25px;margin-bottom: 10px;">Yearly Income w/ Expense Reports<br>Year  <?php echo date('Y', $mycur); ?></header></center>
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     $sqlinventory = "SELECT * FROM transaction_table JOIN employee on encoder = employee.employee_fname  WHERE YEAR(`transaction_sched`)=YEAR( CURRENT_DATE ) ORDER BY price ASC";
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM parts_table WHERE YEAR(`item_transaction_sched`)=YEAR( CURRENT_DATE ) ORDER BY item_total ASC";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     while ($row = mysqli_fetch_array($resultinventory)) {
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM expense_table WHERE YEAR(`expense_date`)=YEAR( CURRENT_DATE ) ORDER BY expense_price ASC";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     while ($row = mysqli_fetch_array($resultinventory)) {
                      ...
                      ?>
                        ...
                        <footer><?php
                            $totalsql = "SELECT SUM(price) as totals FROM transaction_table WHERE YEAR(`transaction_sched`)=YEAR( CURRENT_DATE )";
                            $totalresult= mysqli_query($con,$totalsql);
                            $totaldata = mysqli_fetch_assoc($totalresult);
                            //echo $data['total'];
                            $totalsum = $totaldata['totals'];

                            $totalpartssql = "SELECT SUM(item_total) as totalsparts FROM parts_table WHERE YEAR(`item_transaction_sched`)=YEAR( CURRENT_DATE )";
                            ...
                            //haha
                             ...
                             $totalexpensesql = "SELECT SUM(expense_price) as totalexpense FROM expense_table WHERE YEAR(`expense_date`)=YEAR( CURRENT_DATE )";
                         ...
                         ?><br>
                             <header style="font-size:25px;background-color: greenyellow;padding: 25px;margin-bottom: 10px;">
                                ...
                                Expenses  = <?php echo number_format($totalexpensesum,'2') ?>
                                   <br>Income this Month of  <?php echo date('F', $mycur);echo " = ",number_format($managetotal2,'2') ?></header>
                         </footer>
