...
<head>
    ...
    <center><header style="font-size:25px;background-color: greenyellow;padding: 25px;margin-bottom: 10px;">Yearly Income w/ Expense Reports<br>Year  <?php echo date('Y', $mycur2); ?></header></center>
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     $sqlinventory = "SELECT * FROM transaction_table JOIN employee on encoder = employee.employee_fname  WHERE YEAR(`transaction_sched`)='$test2' ORDER BY price ASC";
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM parts_table WHERE YEAR(`item_transaction_sched`)='$test2' ORDER BY item_total ASC";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     if(mysqli_num_rows($resultinventory)>0){
                        while ($row = mysqli_fetch_array($resultinventory)) {
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM expense_table WHERE YEAR(`expense_date`)='$test2' ORDER BY expense_price ASC";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     if(mysqli_num_rows($resultinventory)>0){
                        while ($row = mysqli_fetch_array($resultinventory)) {
                      ...
                      ?>
                        ...
                        <footer><?php
                            $totalsql = "SELECT SUM(price) as totals FROM transaction_table WHERE YEAR(`transaction_sched`)='$test2'";
                            $totalresult= mysqli_query($con,$totalsql);
                            $totaldata = mysqli_fetch_assoc($totalresult);
                            //echo $data['total'];
                            $totalsum = $totaldata['totals'];

                            $totalpartssql = "SELECT SUM(item_total) as totalsparts FROM parts_table WHERE YEAR(`item_transaction_sched`)='$test2'";
                            ...
                            //haha
                             ...
                             $totalexpensesql = "SELECT SUM(expense_price) as totalexpense FROM expense_table WHERE YEAR(`expense_date`)='$test2'";
                    ...
                    //      echo "<h4 style='font-size:25px;background-color: greenyellow;'> Total Items: ", $numofitems,"</h4>";
                         ...
                         ?><br>
                         <?php
                            if($totalsum > 0 && $totalpartsum > 0 && $totalexpensesum > 0){
                                  ?>
                                ...
                                Expenses  = <?php echo number_format($totalexpensesum,'2') ?>
                                   <br>Income this Year of  <?php echo date('Y', $mycur);echo " = ",number_format($managetotal2,'2') ?></header>
                                <?php   

                            }elseif($totalsum > 0 && $totalpartsum >0){
                                  ?>
                                <header style="font-size:25px;background-color: greenyellow;padding: 25px;margin-bottom: 10px;">
                                Labor & Services = <?php echo number_format($totalsum,'2') ?><br>
                                Parts & Materials  = <?php echo number_format($totalpartsum,'2') ?>
                                   <br>Income this Year of  <?php echo date('Y', $mycur);echo " = ",number_format($managetotal2,'2') ?></header>
                                <?php   
                            }elseif($totalsum > 0 && $totalexpensesum){
                                   ?>
                                <header style="font-size:25px;background-color: greenyellow;padding: 25px;margin-bottom: 10px;">
                                Labor & Services = <?php echo number_format($totalsum,'2') ?><br>
                                Expenses  = <?php echo number_format($totalexpensesum,'2') ?>
                                ...
                                <?php   
                                     ...
                                     ?>
                                <header style="font-size:25px;background-color: greenyellow;padding: 25px;margin-bottom: 10px;">
                                Labor & Services = <?php echo number_format($totalsum,'2') ?>
                                   <br>Income this Year of  <?php echo date('Y', $mycur);echo " = ",number_format($managetotal2,'2') ?></header>
                                <?php  
                            }elseif($totalpartsum >0){
                                      ?>
                                <header style="font-size:25px;background-color: greenyellow;padding: 25px;margin-bottom: 10px;">
                                Parts & Materials  = <?php echo number_format($totalpartsum,'2') ?>
                                   <br>Income this Year of  <?php echo date('Y', $mycur);echo " = ",number_format($managetotal2,'2') ?></header>
                                <?php
                                }elseif($totalexpensesum >0){
                                      ?>
                                <header style="font-size:25px;background-color: greenyellow;padding: 25px;margin-bottom: 10px;">
                                Expenses  = <?php echo number_format($totalexpensesum,'2') ?>
                                  </header>
                                <?php  
                                }elseif($totalpartsum >0 && $totalexpensesum >0){
                                    ?>
                                <header style="font-size:25px;background-color: greenyellow;padding: 25px;margin-bottom: 10px;">
                                Parts & Materials  = <?php echo number_format($totalpartsum,'2') ?><br>
                                Expenses  = <?php echo number_format($totalexpensesum,'2') ?>
                                ...
                                <?php   
                                     ...
                                     ?>
                                <header style="font-size:25px;background-color: greenyellow;padding: 25px;margin-bottom: 10px;">
                                No Record Found  = <?php echo number_format($totalexpensesum,'2') ?><br>
                                </header>
                                <?php   
                                }
