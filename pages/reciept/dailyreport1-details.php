...
<head>
  ...
  <link href="/dist/output.css" rel="stylesheet">
	...
	<center><header style="font-size:25px;background-color: greenyellow;padding: 20px;margin-bottom: 5px;">Daily Reports<br>Chosen Date is  <?php echo date('F, jS, Y', $mycur2); ?></header></center>
             ...
             <tbody>
                ...
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM transacttable JOIN users on custid = users.id JOIN cardata on carid = cardata.id WHERE transact_sched = '$chosendate;'";
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM transaction_table JOIN employee on encoder = employee.employee_fname WHERE transaction_sched = '$chosendate;'";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     while ($row = mysqli_fetch_array($resultinventory)) {
                      $prodname = $row['name'];
                       $prodprice = $row['price'];
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM parts_table WHERE item_transaction_sched = '$chosendate;'";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     while ($row = mysqli_fetch_array($resultinventory)) {
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM expense_table WHERE expense_date = '$chosendate;'";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     while ($row = mysqli_fetch_array($resultinventory)) {
                      ...
                      ?>
                        ...
                        <footer><?php
                            ...
                            $totalpartssql = "SELECT SUM(item_total) as totalsparts FROM parts_table WHERE item_transaction_sched = '$chosendate;'";
