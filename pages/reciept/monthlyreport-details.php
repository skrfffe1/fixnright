...
<head>
  ...
  <link href="/dist/output.css" rel="stylesheet">
	...
	<center><header style="font-size:25px;background-color: greenyellow;padding: 25px;margin-bottom: 10px;">Monthly  Reports<br>Month of  <?php echo date('F', $mycur); ?></header></center>
             ...
             <tbody>
                ...
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM transacttable JOIN users on custid = users.id JOIN cardata on carid = cardata.id WHERE MONTH(`transact_sched`)=MONTH( CURRENT_DATE )";
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM transaction_table JOIN employee on encoder = employee.employee_fname WHERE MONTH(`transaction_sched`)=MONTH( CURRENT_DATE )";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     while ($row = mysqli_fetch_array($resultinventory)) {
                      $prodname = $row['name'];
                       $prodprice = $row['price'];
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM parts_table WHERE MONTH(`item_transaction_sched`)=MONTH( CURRENT_DATE )";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     while ($row = mysqli_fetch_array($resultinventory)) {
