...
<head>
    ...
    <title>Monthly Reports PDF</title>

    <center><header style="font-size:25px;background-color: greenyellow;padding: 20px;margin-bottom: 5px;">Monthly Reports<br>Month of  <?php echo date('F , Y', $mycur2); ?></header></center>
             ...
             <tbody>
                ...
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM transacttable JOIN users on custid = users.id JOIN cardata on carid = cardata.id WHERE MONTH(`transact_sched`)='$test' AND YEAR(`transact_sched`) ='$test2'";
                     ...
                     while ($row = mysqli_fetch_array($resultinventory)) {
                      ...
                      echo "<td>".$full."</td>";
                       echo "<td>".$full1."</td>";
                      echo "<td>".$prodname."</td>";
                       echo '<td>';echo number_format($temps,'2');echo'</td>';
                      echo '</tr>';
                      }
                      ?>
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM transaction_table JOIN employee on encoder = employee.employee_fname WHERE MONTH(`transaction_sched`)='$test' AND YEAR(`transaction_sched`) ='$test2'ORDER BY price ASC";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     while ($row = mysqli_fetch_array($resultinventory)) {
                      $prodname = $row['name'];
                       $prodprice = $row['price'];
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM parts_table WHERE MONTH(`item_transaction_sched`)='$test' AND YEAR(`item_transaction_sched`) ='$test2' ORDER BY item_total ASC";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     while ($row = mysqli_fetch_array($resultinventory)) {
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM expense_table WHERE MONTH(`expense_date`)='$test' AND YEAR(`expense_date`) = '$test2'";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     while ($row = mysqli_fetch_array($resultinventory)) {
                      ...
                      ?>
                        ...
                        <footer><?php
                            $totalsql = "SELECT SUM(price) as totals FROM transaction_table WHERE MONTH(`transaction_sched`)='$test' AND YEAR(`transaction_sched`) = '$test2'";
                            $totalresult= mysqli_query($con,$totalsql);
                            $totaldata = mysqli_fetch_assoc($totalresult);
                            //echo $data['total'];
                            $totalsum = $totaldata['totals'];

                            $totalpartssql = "SELECT SUM(item_total) as totalsparts FROM parts_table WHERE MONTH(`item_transaction_sched`)='$test' AND YEAR(`item_transaction_sched`) = '$test2'";
                            ...
                            //haha
                             ...
                             $totalexpensesql = "SELECT SUM(expense_price) as totalexpense FROM expense_table WHERE MONTH(`expense_date`)='$test'AND YEAR(`expense_date`) = '$test2'";
