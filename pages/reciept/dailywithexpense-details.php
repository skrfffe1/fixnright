...
<head>
  ...
  <link href="/dist/output.css" rel="stylesheet">
	...
	<center><header style="font-size:25px;background-color: greenyellow;padding: 25px;margin-bottom: 10px;">Daily Income w/ Expense Reports<br><br>Today is  <?php echo date('F, jS, Y', $mycur); ?></header></center>
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     $sqlinventory = "SELECT * FROM transaction_table JOIN employee on encoder = employee.employee_fname WHERE transaction_sched = '$currentdate' ORDER BY price ASC";
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM parts_table WHERE item_transaction_sched = '$currentdate' ORDER BY item_total ASC";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     while ($row = mysqli_fetch_array($resultinventory)) {
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM expense_table WHERE expense_date = '$currentdate' ORDER BY expense_price ASC";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     while ($row = mysqli_fetch_array($resultinventory)) {
