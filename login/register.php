...
$username = $password = $confirm_password = $email = $firstname = $lastname = $phone = "";
$username_err = $password_err = $confirm_password_err = $email_err = $firstname_err = $lastname_err = $phone_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["phone"]))){
        ...
        $phone_err = "Phone Number exist in 11 digits.";
    }else{
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($email_err) && empty($firstname_err) && empty($lastname_err) && empty($phone_err)){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssss", $param_username, $param_password , $param_email , $param_firstname , $param_lastname, $param_phone);
                            ...
                            <div class="text-center">
                             ...
                             <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-6">
                                       <input type="number" name="phone"  class="form-control form-control-user <?php echo (!empty($phone_err)) ? 'is-invalid' : ''; ?>" placeholder="Phone" value="<?php echo $phone; ?>">
